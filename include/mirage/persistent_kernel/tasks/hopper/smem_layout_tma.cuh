 ...
 */
 ...
 struct smem_tma {
   ...
   __device__ __forceinline__ size_t get_swizzled_offset(size_t logical_idx) const {
      ...
      if (logical_idx == 64 && threadIdx.x == 0) {
        printf("block_idx: %zu\n", block_idx);
        printf("in_block_idx: %zu\n", in_block_idx);
        printf("irow: %zu\n", irow);
        printf("icol: %zu\n", icol);
        printf("offset_in_bank: %zu\n", offset_in_bank);
        printf("phy_offset: %zu\n", phy_offset);
   ...
   __device__ __forceinline__ T *operator()(size_t logical_idx_row,
     ...
     size_t logical_idx = logical_idx_row * STRIDE_ROW + logical_idx_outer_col * STRIDE_OUTER_COL + logical_idx_inner_col;
