 ...
 */
 ...
 __device__ __forceinline__ void
   ...
   constexpr int TMA_TRANS_BYTES_NORM_WEIGHT = sizeof(T) * BATCH_SIZE * TILE_SIZE;
   constexpr int TMA_TRANS_BYTES_LINEAR_WEIGHT = sizeof(T) * TILE_SIZE * OUTPUT_SIZE;
   ...
   constexpr size_t SHARED_LINEAR_WEIGHT_BUFFER_OFFSET =
    ...
    constexpr size_t SHARED_MUL_OUTPUT_BUFFER_OFFSET =
        SHARED_LINEAR_WEIGHT_BUFFER_OFFSET + sizeof(T) * Kstages * TILE_SIZE * OUTPUT_SIZE;
   ...
   constexpr size_t SHARED_MM_OUTPUT_OFFSET =
       SHARED_ELEMENTARY_UNARY_OUTPUT_OFFSET + sizeof(T) * BATCH_SIZE * OUTPUT_SIZE;
   ...
   constexpr size_t SHARED_REDUCTION_OUTPUT_OFFSET =
    ...
    constexpr size_t SHARED_INPUT_BARRIER_OFFSET = (SHARED_OUTPUT_OFFSET + sizeof(T) * BATCH_SIZE * OUTPUT_SIZE + 7) / 8 * 8;
  ...
  T *shared_element_unary_output = (T *)(smem + SHARED_ELEMENTARY_UNARY_OUTPUT_OFFSET);
  // mm output
  ...
  T *shared_output = (T *)(smem + SHARED_OUTPUT_OFFSET);
   ...
   using NormWeightSmem = smem_row<T, B, M, S, BATCH_SIZE, TILE_SIZE, TILE_SIZE, false>;
  ...
  using B_DESC = wgmma::mma_descriptor<LinearWeightSmem>;
   ...
   using MmaOutputSmem = smem_row<T, 0, 0, 0, BATCH_SIZE, OUTPUT_SIZE, OUTPUT_SIZE, false>;
   ...
   if (warpgroup_id == NUM_WARPGROUPS - 1) {
      ...
      if (lane_id() == 0 && warp_idx == (NUM_WARPGROUPS * WARPGROUP_WARPS - 4)) {
       ...
       for (int i = 0; i < prefetch; i++) {
         int slot = i % Kstages;
          ...
          set_barrier_transaction_bytes(input_barrier[slot], TMA_TRANS_BYTES_INPUT);
          ...
          set_barrier_transaction_bytes(norm_weight_barrier[slot], TMA_TRANS_BYTES_NORM_WEIGHT);
          tma_norm_weight.tma_cp_async(
              norm_weight_barrier[slot], norm_weight_smem_buffer(0, 0), tma_coords_in);

          linear_weight_smem_buffer.set_ptr(shared_linear_weight +
                                           slot * TMA_TRANS_BYTES_LINEAR_WEIGHT / sizeof(T));
          set_barrier_transaction_bytes(linear_weight_barrier[slot], TMA_TRANS_BYTES_LINEAR_WEIGHT);
     ...
     // wg_increase_regs<160>();
      for (int i = 0; i < num_k; i++) {
       ...
       input_smem.set_ptr(shared_input + (slot)*TMA_TRANS_BYTES_INPUT / sizeof(T));
