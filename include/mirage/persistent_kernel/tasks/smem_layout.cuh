...
struct smem_row {
  ...
  __device__ __forceinline__ T *operator()(size_t logical_idx_row,
    ...
    if constexpr (stored_swizzle) {
      return &base_ptr[logical_idx_row * STRIDE + logical_idx_col];
    } else {
...
struct smem_col {
  ...
  __device__ __forceinline__ T *operator()(size_t logical_idx_row,
                                           size_t logical_idx_col) {
    if constexpr (stored_swizzle) {
      return &base_ptr[logical_idx_col * STRIDE + logical_idx_row];
    } else {
