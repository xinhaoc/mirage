...
public:
    ...
    __device__ static constexpr Config config() {
        ...
        constexpr int INPUT_SIZE = BATCH_SIZE * INPUT_PIPE_MAX * TILE_SIZE * sizeof(T);
        constexpr int WEIGHT_SIZE = TILE_SIZE * WEIGHT_PIPE_MAX * OUTPUT_SIZE sizeof(T);
        constexpr int OUTPUT_SIZE_BYTES = BATCH_SIZE * OUTPUT_SIZE * sizeof(T);
        return Config{
            .input_pages = (INPUT_SIZE + PAGE_SIZE - 1) / PAGE_SIZE,
            .weight_pages = (WEIGHT_SIZE + PAGE_SIZE - 1) / PAGE_SIZE,
            .output_pages = (OUTPUT_SIZE_BYTES + PAGE_SIZE - 1) / PAGE_SIZE,
            .input_lifetime = TensorLifetime::IN_OUTPUT,
            .weight_lifetime = TensorLifetime::COMPUTE,
            .output_lifetime = TensorLifetime::IN_OUTPUT
        };
    }
    
    __device__ static int prefetch(
        Params& params,
        TensorAllocation& input_alloc,
        TensorAllocation& weight_alloc,
        TensorAllocation& output_alloc,
        TensorAllocation& temp_alloc,
        cg::thread_block block)
    {
        
        T* shared_input = (T*)input_alloc.ptr;
        T* shared_weight = (T*)weight_alloc.ptr;
        T* shared_output = (T*)output_alloc.ptr;
        T* zero_buf = (T*)temp_alloc.ptr;
