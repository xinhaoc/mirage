...
__device__ __forceinline__ void multitoken_paged_attention_task_impl_32_64(
  ...
  // assert(NUM_WARPS_Q == 4);

  constexpr int NUM_WARPS_KV = 1;
...
#pragma unroll
  ...
  //     for(int n = 0; n < HEAD_DIM / 16; n++){
  //       for (int frag_idx = 0; frag_idx < 8; frag_idx++) {
  //         printf("result of kv is threadIdx.x %d, n %d, fragidx %d, value %f\n", threadIdx.x, n, frag_idx, o[0][n][frag_idx]);
  //       }
  //     }
  //   }
    __syncthreads();
...
//   }
  ...
  //qk = 64 * 64 -> V = 64 * 128 -> 64 * 128 =

  for(int i = 0; i < HEAD_DIM / 16; i++){
    for(int frag_idx = 0; frag_idx < 4; frag_idx++){
      int row = (threadIdx.x >> 1)  + (lane_idx / 4) + (frag_idx % 2) * 8;
      int col = (lane_idx % 4) * 2 + (frag_idx / 2) * 8 + i * 16;
      float d_global = d[0][(row / 8) % 2];
       o_smem.at(row, col) =  bfloat16(o[0][col/16][frag_idx] / d_global);
       o_smem.at(row, col) =  bfloat16(o[0][col/16][frag_idx + 1] / d_global);
    }
