...
static constexpr int RESERVED_SMEM_SIZE = 16 * 1024; // for metadata and other uses
static constexpr int PAGE_SIZE = 8 * 1024;
static constexpr int MAX_PAGES = (MAX_SMEM_SIZE - RESERVED_SMEM_SIZE) / PAGE_SIZE;
...
public:
    ...
    __device__ void try_release_pages(int page_id, int num_pages, TensorLifetime lifetime) {
        if (threadIdx.x == 0) {
            for (int i = 0; i < num_pages; i++) {
